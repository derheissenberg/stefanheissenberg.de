...
## Step 0: AST-Aware Search (jcodemunch MCP — Preferred)

> **Use this first.** jcodemunch parses the actual AST of your codebase and indexes every symbol with its signature, docstring, and an AI-generated summary. This lets you find symbols by *meaning*, trace call graphs, and navigate large repos in seconds — far beyond what grep can do.

### jcodemunch vs. grep — choose the right tool

| Question type | Tool |
|---|---|
| "Find all callers of `processPayment`" | **jcodemunch** `find_references` |
| "What does the `AuthMiddleware` class do?" | **jcodemunch** `search_symbols` / `get_symbol` |
| "Which files import `UserService`?" | **jcodemunch** `find_importers` |
| "All functions in `src/auth/jwt.py`?" | **jcodemunch** `get_file_outline` |
| "Where does the repo even start?" | **jcodemunch** `get_repo_outline` |
| "Find the literal string `'TODO: migrate'"` | **grep** |
| "Find a regex pattern across files" | **grep** |
| "Discover files by name glob `**/*.test.ts`" | **glob** |

### One-time indexing (incremental — safe to re-run)

Before using any jcodemunch tool, index the workspace. The `incremental: true` flag re-indexes only changed files, so it's fast on subsequent runs.

```
CallMcpTool(
  server: "user-jcodemunch",
  toolName: "index_folder",
  arguments: {
...
> **Already indexed?** Call `list_repos` to see all indexed repos. If your workspace appears, skip re-indexing.
> The `repo` parameter in every tool below is the repo name returned by `list_repos` or `index_folder` (usually the folder name, e.g. `"my-project"`).
...
│       → directories, language breakdown, symbol counts
│
├── Find a function/class/type by name or description?
│   └── search_symbols(repo, query, kind?)
│       → ranked matches with signatures + summaries
│       kind options: "function" | "class" | "method" | "constant" | "type"
│
├── Where is symbol X called / referenced?
│   └── find_references(repo, identifier)
│       → all files that import or reference the identifier
│
├── Who imports a specific file?
│   └── find_importers(repo, file_path)
│       → all files that import from that path
│
├── All symbols defined in a file?
│   └── get_file_outline(repo, file_path)
│       → functions, classes, methods with signatures + summaries
│
└── Full source of a specific symbol?
    └── get_symbol(repo, symbol_id)
        symbol_id comes from search_symbols or get_file_outline results
```
...
CallMcpTool(
  server: "user-jcodemunch",
  toolName: "get_repo_outline",
  arguments: { repo: "my-project" }
)
```

**Find authentication middleware (semantic, by description):**
```
CallMcpTool(
  server: "user-jcodemunch",
...
**Find which files import `UserService`:**
```
CallMcpTool(
  server: "user-jcodemunch",
  toolName: "find_importers",
  arguments: {
    repo: "my-project",
...
**Read the full source of a specific symbol** (use the `symbol_id` from `search_symbols` / `get_file_outline`):
```
CallMcpTool(
  server: "user-jcodemunch",
  toolName: "get_symbol",
  arguments: {
    repo: "my-project",
    symbol_id: "src/auth/jwt.py::verify_token",
    context_lines: 5
  }
)
```

### Recommended workflow for serious codebase navigation

```
1. index_folder(path, incremental: true)   ← once per session (or skip if already done)
2. get_repo_outline(repo)                  ← understand structure
3. search_symbols(repo, query, kind?)      ← find the symbol(s) of interest
4. get_file_outline(repo, file_path)       ← explore a file's full API surface
5. find_references(repo, identifier)       ← trace all usages
6. find_importers(repo, file_path)         ← find all consumers of a module
7. get_symbol(repo, symbol_id)             ← read full source of a symbol
```

Fallback to **grep / semantic search** (Steps 1–6 below) only when:
- The codebase has not been indexed and indexing would take too long
- You need literal text matching (strings, regexes, comments)
- You're searching generated or non-standard files that the AST parser skips
